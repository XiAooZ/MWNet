...
@MODELS.register_module()
class HFFVideoDecoder(HFFDecoder):
    ...
    #     outputs = []
    #     for input in inputs:
    #         output = self._forward_feature(input)
    #         output = self.cls_seg(output)
    ...
    #     loss = dict()
    #     for frame_num in range(len(seg_logits)):
    #         seg_logits[frame_num] = resize(
    #             input=seg_logits[frame_num],
    ...
    #     else:
    #         seg_weight = None
    #     seg_label = seg_label.transpose(0, 1)
    #
    #     if not isinstance(self.loss_decode, nn.ModuleList):
    #         losses_decode = [self.loss_decode]
    #     else:
    #         losses_decode = self.loss_decode
    #     for frame, seg_logit in enumerate(seg_logits):
    #         seg_label_cur = seg_label[frame]
    ...
    #     accuracy_total = []
    #     for frame, seg_logit in enumerate(seg_logits):
    #         accuracy_total.append(accuracy(seg_logit, seg_label[frame], ignore_index=self.ignore_index))
    #     accuracy_mean = sum(accuracy_total) / len(accuracy_total)
    #
    #     loss['acc_seg'] = accuracy_mean
    #     return loss
    ...
    #     else:
    #         size = batch_img_metas[0]['img_shape']
    #     for frame in range(len(seg_logits)):
    #         seg_logits[frame] = resize(
    #             input=seg_logits[frame],
    #             size=size,
    #             mode='bilinear',
    #             align_corners=self.align_corners)
    #     return seg_logits
    ...
    def _stack_batch_gt(self, batch_data_samples: SampleList) -> Tensor:
        ...
        return torch.cat(gt_semantic_segs, dim=0).unsqueeze(1)
...
class OrthogonalLifting(nn.Module):
    def __init__(self, kernel_size=3):
        super().__init__()
        # 更新算子 U：一维卷积（线性变换，无激活）
        self.U = nn.Conv1d(
            in_channels=1,  # 输入通道数（单通道信号）
            out_channels=1,  # 输出通道数
            kernel_size=kernel_size,
            padding=kernel_size // 2,  # 保持输出长度与输入一致
            bias=False  # 简化正交性约束，不加偏置
        )
        # 初始化U的卷积核（随机小值，便于学习）
        nn.init.normal_(self.U.weight, mean=0, std=0.1)

    def forward(self, x):
        """
        分解过程：x -> 近似系数a + 细节系数d
        """
        # 假设x形状为 [batch, 1, length]（batch维+通道维+信号长度）
        batch, ch, length = x.shape

        # 1. 分裂（split）：简单分为偶数索引和奇数索引（实际可更灵活）
        x_even = x[..., ::2]  # 偶数位置：[x0, x2, x4, ...]
        x_odd = x[..., 1::2]  # 奇数位置：[x1, x3, x5, ...]

        # 2. 预测（P）：用x_even预测x_odd，得到细节系数d
        # P是U的转置卷积（卷积核翻转+取负）
        # 提取U的卷积核并翻转（一维卷积核的"转置"等价于翻转）
        u_kernel = self.U.weight  # 形状 [1, 1, kernel_size]
        p_kernel = -torch.flip(u_kernel, dims=[-1])  # 翻转最后一维（信号长度维）并取负

        # 用p_kernel对x_even做卷积，得到预测的x_odd
        x_odd_pred = nn.functional.conv1d(
            x_even, p_kernel, padding=self.U.padding[0], stride=self.U.stride[0]
        )
        d = x_odd - x_odd_pred  # 细节系数 = 实际奇数 - 预测奇数

        # 3. 更新（U）：用d更新x_even，得到近似系数a
        a = x_even + self.U(d)  # 近似系数 = 偶数 + U(d)

        return a, d  # 返回近似系数和细节系数

    def reconstruct(self, a, d):
        """
        重构过程：a + d -> 原始信号x（验证正交性的关键：需完全恢复x）
        """
        # 1. 逆更新：从a和d恢复x_even
        x_even_recon = a - self.U(d)

        # 2. 逆预测：从x_even_recon和d恢复x_odd
        u_kernel = self.U.weight
        p_kernel = -torch.flip(u_kernel, dims=[-1])
        x_odd_pred = nn.functional.conv1d(
            x_even_recon, p_kernel, padding=self.U.padding[0], stride=self.U.stride[0]
        )
        x_odd_recon = x_odd_pred + d

        # 3. 合并偶数和奇数（与分裂步骤相反）
        batch, ch, len_a = a.shape
        len_x = len_a * 2 if len(d.shape[-1]) == len_a else len_a * 2 - 1  # 处理长度对齐
        x_recon = torch.zeros(batch, ch, len_x, device=a.device)
        x_recon[..., ::2] = x_even_recon  # 填充偶数位置
        x_recon[..., 1::2] = x_odd_recon  # 填充奇数位置

        return x_recon


# 测试：验证重构误差（正交性的直观体现是重构误差接近0）
if __name__ == "__main__":
    # 生成随机信号（batch=2, 通道=1, 长度=16）
    x = torch.randn(2, 1, 16)
    model = OrthogonalLifting(kernel_size=3)

    # 分解
    a, d = model(x)

    # 重构
    x_recon = model.reconstruct(a, d)

    # 计算重构误差（理想情况下应接近0，证明正交性约束有效）
    error = torch.norm(x - x_recon)
    print(f"重构误差: {error.item()}")  # 初始随机参数下误差可能较大，训练后应接近0