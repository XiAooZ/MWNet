...
class RandomCrop(BaseTransform):
    ...
    def crop_bbox(self, results: dict) -> tuple:
        ...
        def generate_crop_bbox(img: np.ndarray) -> tuple:
            ...
            if isinstance(img, list):
                margin_h = max(img[0].shape[0] - self.crop_size[0], 0)
                margin_w = max(img[0].shape[1] - self.crop_size[1], 0)
            else:
    ...
    def crop(self, img: np.ndarray, crop_bbox: tuple) -> np.ndarray:
        ...
        if isinstance(img, list):
            for i in range(len(img)):
                img[i] = img[i][crop_y1:crop_y2, crop_x1:crop_x2, ...]
        else:
    ...
    def transform(self, results: dict) -> dict:
        ...
        results['img'] = img
        if isinstance(img, list):
            results['img_shape'] = img[0].shape[:2]
        else:
            results['img_shape'] = img.shape[:2]
        return results
...
@TRANSFORMS.register_module()
class RandomCrop_zhang(BaseTransform):
    """Random crop the image & seg.
    ...
    def crop_bbox(self, results: dict) -> tuple:
        ...
        def generate_crop_bbox(img: np.ndarray) -> tuple:
            ...
            if(np.random.rand() < 0.7):
                ...
                positions = np.where(img == 1)
                coord = list(zip(*positions))
                if coord:
                    random_pos = coord[np.random.randint(len(coord))]
                    margin_h = max(random_pos[0] - self.crop_size[0] // 2, 0)
                    margin_w = max(random_pos[1] - self.crop_size[1] // 2, 0)
                    margin_h = max(0, min(img.shape[0] - self.crop_size[0], margin_h))
                    margin_w = max(0, min(img.shape[1] - self.crop_size[1], margin_w))
        ...
        crop_bbox = generate_crop_bbox(img[...,0])
        '''
        if self.cat_max_ratio < 1.:
            # Repeat 10 times
            for _ in range(10):
...
@TRANSFORMS.register_module()
class GroupRandomFlip(BaseTransform):
    """Randomly flip a sequence of frames (clip) with the same parameters."""

    def __init__(self, prob=0.5):
        self.prob = prob
    def transform(self, results):
        direction = ['horizontal', 'vertical', 'diagonal']
        # 假设输入帧保存在 results['imgs'] 中，是 list[np.ndarray]
        if results['augment'][4, 0] < self.prob:
            results['flip'] = True
            results['flip_direction'] = direction[int(results['augment'][4, 1])]
            results['img'][:,:, 0:3] = mmcv.imflip(results['img'][:,:, 0:3], direction=direction[int(results['augment'][4, 1])])
            results['img'][:, :, 3:6] = mmcv.imflip(results['img'][:, :, 3:6],
                                                     direction=direction[int(results['augment'][4, 1])])
            results['img'][:, :, 6:9] = mmcv.imflip(results['img'][:, :, 6:9],
                                                     direction=direction[int(results['augment'][4, 1])])
            # 如果你还有 mask，比如 segmentation，可能需要一起翻
            if 'gt_seg_map' in results:
                results['gt_seg_map'] = mmcv.imflip(results['gt_seg_map'], direction=direction[int(results['augment'][4, 1])])
...
class RandomFlip(MMCV_RandomFlip):
    ...
    def _flip(self, results: dict) -> None:
        ...
        if isinstance(imgs, list):
            ...
            for cur_img in imgs:
                cur_img = mmcv.imflip(
                    cur_img, direction=results['flip_direction'])
                img.append(cur_img)
            results['img'] = img
...
class RandomBlur(BaseTransform):
    ...
    def transform(self,results: dict) ->dict:
        ...
        if results['augment'][5, 0] <= self.prob:
            if isinstance(results['img'], list):
                img = []
                for i in range(len(results['img'])):
                    img_cur = results['img'][i].astype(np.float32)
                    sigma = 3
                    kernel_size = 3
                    kernel = cv2.getGaussianKernel(kernel_size, sigma)
                    kernel = kernel * kernel.transpose()
                    img_cur = cv2.filter2D(img_cur, -1, kernel)#.astype(np.uint8)
                    img.append(img_cur)
                ...
                kernel = cv2.getGaussianKernel(kernel_size, sigma)
                kernel = kernel * kernel.transpose()
                img[:,:, :3] = cv2.filter2D(img[:,:, :3], -1, kernel)#.astype(np.uint8)
                img[:, :, 3:6] = cv2.filter2D(img[:, :, 3:6], -1, kernel)#.astype(np.uint8)
                img[:, :, 6:9] = cv2.filter2D(img[:, :, 6:9], -1, kernel)#.astype(np.uint8)
            results['img'] = img
        return results
...
import torch
@TRANSFORMS.register_module()
class GroupColorJitter(BaseTransform):
    def __init__(self, prob=0.3):
        self.prob = prob
    def transform(self, results):
        img_ori = results['img']
        augment = results['augment']
        img_list = [img_ori[:, :, 0:3], img_ori[:, :, 3:6], img_ori[:, :, 6:9]]
        img_resultlist = []
        for img in img_list:
            # 1. numpy [H, W, C] -> tensor [C, H, W], float32 in [0,1]
            img_tensor = torch.from_numpy(img).permute(2, 0, 1).float() / 255.0

            # 2. Apply color jitter
            if augment[0, 0] < self.prob:
                img_tensor = adjust_brightness(img_tensor, augment[0, 1])
            if augment[1, 0] < self.prob:
                img_tensor = adjust_contrast(img_tensor, augment[1, 1])
            if augment[2, 0] < self.prob:
                img_tensor = adjust_saturation(img_tensor, augment[2, 1])
            if augment[3, 0] < self.prob:
                img_tensor = adjust_hue(img_tensor, augment[3, 1])

            # 3. Back to numpy [H, W, C], uint8
            img_out = (img_tensor * 255.0).clamp(0, 255).permute(1, 2, 0).numpy()

            img_resultlist.append(img_out)
        img_result = np.concatenate((img_resultlist[0], img_resultlist[1], img_resultlist[2]), axis=2)
...
@TRANSFORMS.register_module()
class PhotoMetricDistortionThreeFrames(BaseTransform):
    def __init__(self, prob=0.3):
        self.prob = prob
    ...
    def brightness(self, img: np.ndarray, augment: np.ndarray) -> np.ndarray:
        if augment[0, 0] < self.prob:
            return self.convert(
                img,
                beta=augment[0, 1])
    ...
    def contrast(self, img: np.ndarray, augment: np.ndarray) -> np.ndarray:
        if augment[1, 0] < self.prob:
            return self.convert(
                img,
                alpha=augment[1, 1])
    ...
    def saturation(self, img: np.ndarray, augment: np.ndarray) -> np.ndarray:
        if augment[2, 0] < self.prob:
            img = mmcv.bgr2hsv(img)
            img[:, :, 1] = self.convert(
                img[:, :, 1],
    ...
    def hue(self, img: np.ndarray, augment: np.ndarray) -> np.ndarray:
        if augment[3, 0] < self.prob:
            img = mmcv.bgr2hsv(img)
            img[:, :,
                0] = (img[:, :, 0].astype(int) +
    ...
    def transform(self, results: dict) -> dict:
        img_ori = results['img']
        #img_ori = img_ori.reshape(img_ori.shape[0], img_ori.shape[1], 3, 3)
        #img_ori = img_ori.transpose(2, 0, 1, 3)
        img_list = [img_ori[:, :, 0:3], img_ori[:, :, 3:6], img_ori[:, :, 6:9]]
        # random brightness
        img_resultlist = []
        for img in img_list:
            img = self.brightness(img, results['augment'])

            # mode == 0 --> do random contrast first
            # mode == 1 --> do random contrast last
            if results['augment'][1, 0] < 2:
                img = self.contrast(img, results['augment'])
            # random saturation
            img = self.saturation(img, results['augment'])

            # random hue
            img = self.hue(img, results['augment'])

            if results['augment'][1, 0] >= 2 and results['augment'][1, 0] < self.prob:
                img = self.contrast(img, results['augment'])
