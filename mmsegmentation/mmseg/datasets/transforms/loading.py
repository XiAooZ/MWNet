...
import os
@TRANSFORMS.register_module()
class LoadVideoThreeFramesFromFiles(BaseTransform):
    def __init__(self,
    ...
    def transform(self, results: dict) -> Optional[dict]:
        def create_lastfram(original_path):
            img_num = int(original_path[-8:-4].lstrip('0'))
            new_img_num = img_num-1
            new_img_num = str(new_img_num).zfill(4)

            # new_path = original_path.replace(str(img_num).zfill(4), new_img_num)
            new_path = os.path.join(os.path.dirname(original_path), new_img_num + '.jpg')
        ...
        frame = []
        frame.append(results['img_path'])

        frame.append(create_lastfram(frame[-1]))
        frame.append(create_lastfram(frame[-1]))
        imgs = []
        for f in frame:
            img_byte = fileio.get(f)
            img = mmcv.imfrombytes(img_byte)
            imgs.append(img)
        try:
            img = np.concatenate((imgs[-1], imgs[-2], imgs[-3]), axis=2)
        except:
...
@TRANSFORMS.register_module()
class LoadVideoOneFramesFromFiles(BaseTransform):
    def __init__(self,
                 to_float32: bool = False,
                 color_type: str = 'color',
                 imdecode_backend: str = 'cv2',
...
@TRANSFORMS.register_module()
class LoadAnnotationsFrames(MMCV_LoadAnnotations):
    def __init__(
        self,
        reduce_zero_label=None,
        backend_args=None,
